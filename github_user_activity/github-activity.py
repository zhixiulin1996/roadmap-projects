...
def one_time_event(event):
    """
    Deal with one-time event and add the corresponding output ot "summary"
    :param event: (dict) the basic information of event
    :return: none
    """
    global summary
...
def cumulative_event(event):
    """
    Deal with cumulative event and use tmp_dict to store information and their count
    :param event: (dict) the basic information of event
    :return: none
    """
...
def generate_event_summary(events):
    """
    1. Divide events into 2 kinds: one-time event and cumulative event (refer to these function, respectively)
    2. Based on tmp_dict, add cumulative event output to summary
    :param events: (list) the recent events of user in json form
    :return: none
    """
    global summary, tmp_dict
    ...
    for key, val in tmp_dict.items():
        if key[0] == "PushEvent":
            ...
            summary.append(
                f"- {'Opened' if key[1]=='opened' else 'Closed'} {val} pull request(s) in {key[2]}."
            )
        elif key[0] == "IssuesEvent":
            summary.append(
                f"- {'Opened' if key[1]=='opened' else 'Closed'} {val} issue(s) in {key[2]}."
            )
        elif key[0] == "IssueCommentEvent":
            summary.append(
                f"- {'Created' if key[1]=='created' else 'Deleted'} {val} comment(s) in {key[2]} issue(s)."
            )
...
def fetch_user_events(username, token=None):
    ...
    # Send http request and exception handling
    ...
    try:
        ...
        code = e.code
        message = error_code.get(
            code, f"HTTP Error {code}"
        )  # if code is not in dictionary, return formatted string
...
def main():
    """
    Main function, Fetch user's event from GitHub API, summarized them and output
    """
    # Fetch user events from server and store to "events"(type: list)
    username = str(sys.argv[1])
    token = (
        str(sys.argv[2]) if len(sys.argv) == 3 else None
    )  # Default token will be none
